...
The catalog retains the six workbook-declared TPDO mapping groups `0x1A00` through `0x1A05` as source metadata and validates that every referenced application index exists and that each map is at most 64 bits. The generated test OD populates only the four mapping records provided by the pinned upstream template (`0x1A00` through `0x1A03`); `0x1A04` and `0x1A05` are not silently invented as additional communication objects. The generated test OD leaves the communication layer disabled by default; mapping activation and COB-ID/node-ID policy require an explicit test harness decision and are not silently enabled in firmware.
