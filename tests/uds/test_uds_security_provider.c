...
int main(void) {
    ...
    assert(uds_security_provider_generate_seed(&provider, 1U, seed, &seed_length,
                                               sizeof(seed), 0U) == UDS_SECURITY_OK);
    ...
    for (uint8_t attempt = 0U; attempt < 2U; ++attempt) {
        assert(uds_security_provider_generate_seed(&provider, 1U, seed, &seed_length,
                                                   sizeof(seed), 2U) == UDS_SECURITY_OK);
        key[0] ^= 0x01U;
    ...
    assert(uds_security_provider_generate_seed(&provider, 1U, seed, &seed_length,
                                               sizeof(seed), 5U) == UDS_SECURITY_OK);
    key[0] ^= 0x01U;
